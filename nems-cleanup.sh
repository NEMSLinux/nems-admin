...
# Generate Unique NEMS SSL and SSH Certificates on First Boot
...
Before=apache2.service ssh.service nagios.service rpimonitor.service

[Service]
Type=oneshot
ExecStart=/bin/bash -c '/usr/local/bin/nems-cert --no-restart && systemctl disable nems-cert-firstboot.service && rm -f /etc/systemd/system/nems-cert-firstboot.service && systemctl daemon-reload'
RemainAfterExit=no
